...
```{r}
head (beta_table_tp[, 1:5])
head (beta_table_nt[, 1:5])
...
auc_meth <- compute_AUC(tumor_table = beta_table_tp, control_table = beta_table_nt, 
...
auc_density <- density(auc_meth, bw = 0.2, from = 0, to = 1, na.rm = T)
...
avg_beta_tp <- apply(beta_table_tp, 1, mean, na.rm = T)
avg_beta_nt <- apply(beta_table_nt, 1, mean, na.rm = T)
## create a flag for differential methylation analysis
dms_flag <- rep(NA, length(auc_meth))
dms_flag[which(auc_meth > 0.8)] <- "hyper"
dms_flag[which(auc_meth < 0.2)] <- "hypo"
## create the table
dms_table <- cbind(illumina450k_hg19, avg_beta_nt, avg_beta_tp, auc_meth, dms_flag)
head (dms_table)
...
cor_auc_betadiff <- cor(dms_table$auc_meth, (dms_table$avg_beta_tp-dms_table$avg_beta_nt), use = "na")
{smoothScatter(dms_table$auc_meth, (dms_table$avg_beta_tp-dms_table$avg_beta_nt),
...
```{r}
dms_table.srt <- dms_table[order(abs(dms_table$auc_meth-0.5), decreasing = T), ]
head (dms_table.srt)
```
...
beta_table_tp <- beta_table_tp[idx,]
beta_table_nt <- beta_table_nt[idx,]
auc_meth.srt <- auc_meth[idx]
illumina450k_hg19 <- illumina450k_hg19[idx,]
## run segmentation of AUC
dmr_table <- whole_genome_segmentator(tumor_table = beta_table_tp, control_table = beta_table_nt, 
                                      auc_vector = auc_meth.srt, reference_table = illumina450k_hg19[3:4])
head (dmr_table)
```

You can draw a vulcano plot to identify significant DMRs (GSTP1 is highlighted)

```{r fig.cap= "Vulcano Plot", fig.height=5, fig.width=6}
{plot(dmr_table$avg_beta_diff, -log10(dmr_table$q_value), log = "y", las = 1,  frame.plot = F, pch = 16, col = "grey",
...
sample_score <- compute_z_score(tumor_table = beta_table_tp, control_table = beta_table_nt, 
                                dmr_table = dmr_table, reference_table = illumina450k_hg19[3:4])
```

```{r message=FALSE, warning=FALSE, eval=TRUE}
head (sample_score$z_scores[, 1:5])
```
...
```{r message=FALSE, warning=FALSE, eval=FALSE}
write_output(dmr_table = dmr_table, sample_score = sample_score, path = "~/PRAD.subset")
```
