...
PRAD_TP_subset.srt <- PRAD_TP_subset[idx,]
PRAD_NT_subset.srt <- PRAD_NT_subset[idx,]
PRAD_auc.srt <- PRAD_auc[idx]
illumina450k_hg19.srt <- illumina450k_hg19[idx,]
## run segmentation of AUC
dmr_table <- whole_genome_segmentator(tumor_table = PRAD_TP_subset.srt, control_table = PRAD_NT_subset.srt,
                                      auc_vector = PRAD_auc.srt, reference_table = illumina450k_hg19.srt[3:4])
head(dmr_table)
```
...
```{r  fig.height=5, fig.width=6}
{plot(dmr_table$avg_beta_diff, -log10(dmr_table$q_value), main= "Volcano Plot", log = "y", las = 1,
...
```{r message=FALSE, warning=FALSE, eval=FALSE}
PRAD_sample_score <- compute_z_scores(tumor_table = PRAD_TP_subset.srt, control_table = PRAD_NT_subset.srt,
                                dmr_table = dmr_table, reference_table = illumina450k_hg19.srt[3:4],
                                min_size = 6)
```
...
In our parameter study, we observed that our method is very sensible to changes in F. F is the parameter used to split the total AUC signal variance in two parts: the variance of not differentially methylated state and the variance of differentially methylated states.
Here we will see how Rocker-meth results are affected by the parameter F from its standard value (0.4) to different values.
Based on our study, we expect that lowering F will increase specificity at the cost of sensitivity. The opposite situation occurs for larger values of F.

```{r fig.height=5, fig.width=6, message=FALSE, warning=FALSE}

## run Rocker-meth in stringend mode, F = 0.2 (i.e., ratiosd)
dmr_table_hi_specificity <- whole_genome_segmentator(tumor_table = PRAD_TP_subset.srt, control_table = PRAD_NT_subset.srt,
                                      auc_vector = PRAD_auc.srt, reference_table = illumina450k_hg19.srt[3:4],
                                      ratiosd = 0.2)
## run Rocker-meth in high sensitivity mode, F = 0.6
dmr_table_hi_sensitivity <- whole_genome_segmentator(tumor_table = PRAD_TP_subset.srt, control_table = PRAD_NT_subset.srt,
                                      auc_vector = PRAD_auc.srt, reference_table = illumina450k_hg19.srt[3:4],
                                      ratiosd = 0.6)
dat_n_seg <- c(dim(dmr_table_hi_specificity)[1],
               dim(dmr_table)[1],
               dim(dmr_table_hi_sensitivity)[1])
...
nsites <- 15
mybox <- list(-log10(dmr_table_hi_specificity$p_value[which(dmr_table_hi_specificity$nsites<nsites & dmr_table_hi_specificity$state != 2)]),
              -log10(dmr_table$p_value[which(dmr_table$nsites<nsites & dmr_table$state != 2)]),
              -log10(dmr_table_hi_sensitivity$p_value[which(dmr_table_hi_sensitivity$nsites<nsites & dmr_table_hi_sensitivity$state != 2)]))
...
Here we will see a strategy that can be used to identify DMRs specific to molecular subtypes. Also in this case
we will consider the PRAD dataset. characterized by 8 molecular subytpes (TCGA-PRAD Consortium, Cell 2015).
...
Subtype specific DMRs can be identified as those showing different Z-score statistics across the different subtypes.
First, we match methylation sample IDs with patient ID.

```{r message=FALSE, warning=FALSE}
subtypes <- names(table(PRAD_molecular_subtypes$Subtype))
subtype_data <- rep(NA, ncol(PRAD_TP_subset.srt))
for (subtype in subtypes) {
  ix <- which(substr(colnames(PRAD_TP_subset.srt), 1, 12) %in% PRAD_molecular_subtypes$PATIENT_ID[which(PRAD_molecular_subtypes$Subtype == subtype)])
