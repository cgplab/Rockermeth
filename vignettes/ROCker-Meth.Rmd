...
na_threshold refers to the fraction of samples permitted to be NA at a give
site (no beta value available). By setting it to 0, we will calculate AUC only
...
based on a Heterogeneous Shifting Level Model that segments AUC scores to
classify sites in stretches of hyper-methylation, no differential methylation
or hypo-methylation. To do that, we will use the **whole_genome_segmentator**
function.
...
differentially methylated states.
Here we will see how Rocker-meth results are affected by changing parameter F
from its standard value (0.4) to different values.
Based on our study, we expect that lowering F will increase specificity at the
cost os sensitivity. The opposite will occur for larger values of F.

```{r message=FALSE, warning=FALSE}

## run Rocker-meth in stringend mode, F = 0.2 (i.e., ratiosd)
dmr_table_hi_specificity <- whole_genome_segmentator(tumor_table = PRAD_TP_subset, control_table = PRAD_NT_subset,
                                      auc_vector = PRAD_auc.srt, reference_table = illumina450k_hg19[3:4],
                                      ratiosd = 0.2)
## run Rocker-meth in high sensitivity mode, F = 0.6
dmr_table_hi_sensitivity <- whole_genome_segmentator(tumor_table = PRAD_TP_subset, control_table = PRAD_NT_subset,
                                      auc_vector = PRAD_auc.srt, reference_table = illumina450k_hg19[3:4],
                                      ratiosd = 0.6)
...
names(dat_n_seg) <- c("hi spec (F=0.2)",
                      "standard (F=0.4)",
                      "hi sens (F=0.6)")
barplot(dat_n_seg, border = NA, las = 1, ylab = "number of segments", beside = T)
```

We can nicely demostrate that modulating F affects sensibility/sensitivity
trade-off by looking at the distribuition of p-values of DMRs in the three
settings.
...
nsites <- 15
mybox <- list(-log10(dmr_table_hi_specificity$p_value[which(dmr_table_hi_specificity$nseg<nsites & dmr_table_hi_specificity$state != 2)]),
              -log10(dmr_table$p_value[which(dmr_table$nseg<nsites & dmr_table$state != 2)]),
              -log10(dmr_table_hi_sensitivity$p_value[which(dmr_table_hi_sensitivity$nseg<nsites & dmr_table_hi_sensitivity$state != 2)]))
names(mybox) <- names(dat_n_seg)
{
  par (mar = c(7.5, 4, 4, 2))
  boxplot(mybox, varwidth = T, las = 2, ylab = "-log10(P-value)", frame.plot = F, pch = 19, cex = .6)
}
```
...
First, we match methylation IDs with TCGA patiend ID.
...
A reasonable strategy to identify candidates subtype specific DMRs is by means of one-way ANOVA test.
...
## As an exmaple, plot the firsts
{
  ...
  for (i in 1: 4) {
    mymax <- round(max(abs(PRAD_sample_score$z_scores[j[i], ]))/10)*10
      boxplot(PRAD_sample_score$z_scores[j[i], ] ~ subtype_data, varwidth = T, outline = F, ylim = c(-mymax, mymax),
        frame.plot = F, las = 2, ylab = "RockerSS (Z-score)",
...
In these boxplots, we see that i) first and third (left) DMRs tends to be more hyper-methylated in ERG and SPOP samples, second and four
are clearly hypo-methylated in FOXA1 subtypes and not in the other subtypes.
