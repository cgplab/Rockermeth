...
The package include also a fuction (**write_output**) to easily produce a bed file
...
by a tailored Heterogeneous Hidden Markov Model based algorithm and samples supporting DMRs.
...
dmr_table <- find_dmrs(tumor_table = PRAD_TP_subset, control_table = PRAD_NT_subset,
                                      auc_vector = PRAD_auc, reference_table = illumina450k_hg19[3:4])
```
...
```{r message=FALSE, warning=FALSE, eval=FALSE}
write_output(path = "~/PRAD.subset", dmr_table = dmr_table, z_score_table = PRAD_sample_score)
```
...
dmr_table_hi_specificity <- find_dmrs(tumor_table = PRAD_TP_subset, control_table = PRAD_NT_subset,
                                      auc_vector = PRAD_auc, reference_table = illumina450k_hg19[3:4],
                                      ratiosd = 0.2)
## run Rocker-meth in high sensitivity mode, F = 0.6
dmr_table_hi_sensitivity <- find_dmrs(tumor_table = PRAD_TP_subset, control_table = PRAD_NT_subset,
                                      auc_vector = PRAD_auc, reference_table = illumina450k_hg19[3:4],
                                      ratiosd = 0.6)
