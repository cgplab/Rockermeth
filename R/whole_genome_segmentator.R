...
whole_genome_segmentator <- function(tumor_table, control_table, auc_vector,
  reference_table, max_distance = Inf, length_cutoff = 5, pt_start = 0.05,
  ...
  for (n in seq_along(chromosomes)) {
    ...
    meth_states <- meth_state_finder(auc_subset, coordinates, auc_sd,
      length_cutoff, pt_start, normdist, ratiosd, mu, use_trunc)

    # 2) find segments
    single_chr_segs <- segmentator(t_beta_mean_subset,
                                   c_beta_mean_subset,
                                   meth_states,
                                   coordinates,
                                   max_distance)

    all_chr_segs[[n]] <- tibble::add_column(single_chr_segs, chr=chr, .before="start")
  }
