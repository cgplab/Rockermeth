...
write_dmr <- function(dmr_table, path, qvalue_thr = 0.05){
    ...
    out_table_bed_hypo <- dplyr::mutate(out_table_bed_hypo, start = start - 1, q_value = -log10(q_value))
    out_table_bed_hypo <- dplyr::select(out_table_bed_hypo, chr, start, end, nsites, q_value)

    out_table_bed_hyper <- dmr_table
    out_table_bed_hyper <- dplyr::filter(out_table_bed_hyper, state == 3)
    out_table_bed_hyper <- dplyr::mutate(out_table_bed_hyper, start = start - 1, q_value = -log10(q_value))
    out_table_bed_hyper <- dplyr::select(out_table_bed_hyper, chr, start, end, nsites, q_value)
